...
namespace Halwaytovegas\ClassifiedListingModifications\Widgets;
...
class ModifiedListingSlider extends ListingSliderSettings {
    ...
    public function __construct( $data = [], $args = null ) {
        $this->rtcl_name = __( 'Listing Slider Bro Modified', 'classified-listing-toolkits' );
        $this->rtcl_base = 'rtcl-listing-slider';
        parent::__construct( $data, $args );
        add_filter( 'rtcl_get_template', [$this, 'modify_listing_template'], 10, 3 );
    }

    public function modify_listing_template( $located, $template_name, $args ) {
		if( $template_name == 'elementor/listing-ads-slider/grid/style-1' ) {
			$located = LISTING_TEMPLATES_FOLDER . 'grid/style-1.php';
		}
        return $located;
    }
    ...
    public function widget_query_args() {
        ...
        $pricings         = Helpers::get_pricing_packages( [], false, false, true );
        ...
        if ( ! empty( $promotion_in ) && is_array( $promotion_in ) ) {
            ...
            foreach ( $promotion_in as $promotion ) {
                if ( isset( $pricings[$promotion] ) && ! empty( $pricings[$promotion] ) ) {
                    $post_object    = get_page_by_path( $promotion, OBJECT, 'rtcl_pricing' );
                    $post_id        = isset( $post_object->ID ) ? $post_object->ID : 0;
                    $meta_queries[] = [
                        'key'     => 'rtcl_pricing_packages',
                        'compare' => 'IN',
                        'value'   => $post_id,
                        'type'    => 'NUMERIC'
                    ];
        ...
        if ( $count_meta_queries ) {
            $the_args['meta_query'] = ( $count_meta_queries > 0 ) ? array_merge( ['relation' => 'OR'], $meta_queries ) : $meta_queries;
        }
    ...
    protected function render() {
        ...
        Functions::get_template( $data['template'], $data, '', $data['default_template_path'] ); //elementor/listing-ads-slider/grid/style-1
        wp_reset_postdata();
        if ( ! $settings['rtcl_show_price_unit'] ) {
