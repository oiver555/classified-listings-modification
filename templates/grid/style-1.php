    ...
    ?>
        ...
        <?php
            ...
            if ( $instance['rtcl_show_image'] ) {
                ...
                if ( $the_thumbnail ) {
                    $author_id = $listing->get_author_id(); // Get the user ID of who posted the ad
                    $user_data = get_userdata( $author_id );
                    $user_role = isset( $user_data->roles ) ? $user_data->roles : [];
                    ob_start();
                    $pricing_badge_html = ob_get_clean();

                    if ( in_array( 'administrator', $user_role ) ) {
                        // If user ID is 1, wrap thumbnail + badge + overlay
                        $thumbnail_html = sprintf(
                            "<div class='image-overlay-wrapper'>
                            %s
                            %s
                            <div class='overlay-text'>Available Ad Space</div>
                        </div>",
                            $pricing_badge_html, // The badge goes first
                            $the_thumbnail // Then the image
                        );
                    } else {
                        // Just normal image (with badge if you want)
                        $thumbnail_html = sprintf(
                            "<div class='real-ad-no-overlay'>
                            %s
                            %s
                        </div>",
                            $pricing_badge_html,
